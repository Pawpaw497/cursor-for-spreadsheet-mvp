...
#### [agent-preview-abort-loop.plan.md](../../.cursor/plans/agent-preview-abort-loop.plan.md)
...
| ----------------------- | -------------------------------------------------- |
| `define-preview-state`  | PreviewRecord + abort outcome                      |
| `split-dry-run-commit`  | dry-run preview 与 commit 分离                        |
| `extend-langgraph-flow` | LangGraph validate/dry-run/confirm/abort/revise 分支 |
| `wire-ui-confirmation`  | 前端 confirm / abort / revise 控制                     |
| `regression-and-docs`   | 回归测试 + FEATURES / AGENT_IMPROVEMENTS               |


**理由：** 将你确认的 Cursor-style `abort = preview 阶段 Undo all` 语义落成显式 Agent 状态机；适合作为当前 LangGraph 编排之后的下一阶段增强。
...
| -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | ----------------------- |
| [agent-preview-abort-loop.plan.md](../../.cursor/plans/agent-preview-abort-loop.plan.md)                                               | agent-preview-abort-loop               | **pending** ×5（见 P2）    |
